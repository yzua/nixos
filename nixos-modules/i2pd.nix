...
in
{
  options.mySystem.i2pd = {
    ...
    notransit = mkBoolOption true "Disable transit tunnel participation to reduce relay traffic.";

    bandwidth =
      mkNullableOption lib.types.int null
        "Router bandwidth cap in KB/s. Null keeps the upstream i2pd default.";

    port =
      mkNullableOption lib.types.port null
        "Fixed external I2P transport port. Null lets i2pd choose automatically.";

    openFirewall = mkBoolOption false "Open TCP/UDP firewall rules for the configured i2pd port.";
  };
