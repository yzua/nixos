...
in
{
  options.mySystem.android = {
    enable = lib.mkEnableOption "Android device and emulator support (ADB, Fastboot, Android Studio)";
  };

  config = lib.mkIf config.mySystem.android.enable {
    environment = {
      systemPackages = [
