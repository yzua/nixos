...
analyze() {
    ...
    # Find packages that are both in PROG_MAP and PKG_MAP
    join -t $'\t' -1 1 -2 1 \
        <(cut -f1 "$PROG_MAP_FILE" | sort -u) \
        <(cut -f1 "$PKG_MAP_FILE" | sort -u) 2>/dev/null | while read -r pkg; do
            [[ -n "$pkg" ]] || continue
            local prog_file pkg_file
            prog_file=$(grep "^${pkg}"$'\t' "$PROG_MAP_FILE" | head -1 | cut -f2)
            pkg_file=$(grep "^${pkg}"$'\t' "$PKG_MAP_FILE" | head -1 | cut -f2)
            print_error "Package '$pkg' is both:"
            echo "    • Enabled via programs.$pkg in $prog_file"
            echo "    • Explicitly installed in: $pkg_file"
        done

    # Count conflicts
    local conflict_count
    conflict_count=$(join -t $'\t' -1 1 -2 1 \
        <(cut -f1 "$PROG_MAP_FILE" | sort -u) \
        <(cut -f1 "$PKG_MAP_FILE" | sort -u) 2>/dev/null | wc -l)
    errors=$((errors + conflict_count))

    # Check service modules vs explicit packages
    join -t $'\t' -1 1 -2 1 \
        <(cut -f1 "$SVC_MAP_FILE" | sort -u) \
        <(cut -f1 "$PKG_MAP_FILE" | sort -u) 2>/dev/null | while read -r pkg; do
            [[ -n "$pkg" ]] || continue
            local svc_file pkg_file
            svc_file=$(grep "^${pkg}"$'\t' "$SVC_MAP_FILE" | head -1 | cut -f2)
            pkg_file=$(grep "^${pkg}"$'\t' "$PKG_MAP_FILE" | head -1 | cut -f2)
            print_error "Package '$pkg' is both:"
            echo "    • Enabled via services.$pkg in $svc_file"
            echo "    • Explicitly installed in: $pkg_file"
        done

    local svc_conflict_count
    svc_conflict_count=$(join -t $'\t' -1 1 -2 1 \
        <(cut -f1 "$SVC_MAP_FILE" | sort -u) \
        <(cut -f1 "$PKG_MAP_FILE" | sort -u) 2>/dev/null | wc -l)
    ...
    pkg_count=$(cut -f1 "$PKG_MAP_FILE" 2>/dev/null | sort -u | wc -l || echo 0)
    prog_count=$(cut -f1 "$PROG_MAP_FILE" 2>/dev/null | sort -u | wc -l || echo 0)
    svc_count=$(cut -f1 "$SVC_MAP_FILE" 2>/dev/null | sort -u | wc -l || echo 0)
