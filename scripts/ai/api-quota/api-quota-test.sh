...
assert_eq "$(display_pct 7)" "07" "display percent pads single digits"
assert_eq "$(display_pct '?')" "--" "display percent renders unknown as dashes"
assert_eq "$(build_status_text 95 '?' 7)" "Z95 C-- O07" "status text summarizes provider states"
...
assert_contains "$tip_epoch" "5h used" "tooltip contains 5h usage label"
assert_contains "$tip_epoch" "23.4%" "tooltip contains 5h usage value"
assert_contains "$tip_epoch" "7d used" "tooltip contains 7d usage label"
assert_contains "$tip_epoch" "left 55%" "tooltip contains 7d remaining value"
