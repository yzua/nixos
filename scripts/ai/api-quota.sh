#!/usr/bin/env bash
# api-quota.sh - Noctalia bar widget: Z.ai + Claude Max usage (parseJson mode).
set -euo pipefail

ZAI_KEY_FILE="/run/secrets/zai_api_key"
ZAI_ENDPOINT="https://bigmodel.cn/api/monitor/usage/quota/limit"
...
fetch_zai() {
	...
	local tip
	tip="<b>Z.ai GLM Coding</b>"
	# Only show token counts when API returns actual values
	if [[ -n "$used" && -n "$limit" ]]; then
		tip+="${NL}Tokens: $(format_tokens "$used") / $(format_tokens "$limit")"
	fi
	tip+="${NL}Used: $(printf "%.1f" "$pct")%  Left: ${remaining}%"
	if [[ -n "$reset_ms" && "$reset_ms" != "null" ]]; then
		tip+="${NL}Resets in: $(time_until "$((reset_ms / 1000))")"
	fi
	...
	[[ -n "$mcp_used" && -n "$mcp_limit" ]] && tip+="${NL}MCP: ${mcp_used}/${mcp_limit} searches"
...
fetch_claude() {
	...
	local tip
	tip="<b>Claude Max (20x)</b>"
	tip+="${NL}5h: $(printf "%.1f" "$five_h")% used  ${remaining}% left"
	if [[ -n "$five_h_reset" && "$five_h_reset" != "null" ]]; then
		local epoch
		epoch=$(date -d "$five_h_reset" +%s 2>/dev/null) || true
		[[ -n "$epoch" ]] && tip+="${NL}Resets in: $(time_until "$epoch")"
	fi
	if [[ -n "$seven_d" ]]; then
		local sd_int=$(($(printf "%.0f" "$seven_d")))
		tip+="${NL}7d: $(printf "%.1f" "$seven_d")% used  $((100 - sd_int))% left"
	fi
...
icon="activity"
for pct in "$zai_pct" "$claude_pct"; do
	if [[ "$pct" =~ ^[0-9]+$ ]]; then
		if ((pct <= 20)); then
			icon="alert-triangle"
			break
...
tooltip="${zai_tip}${NL}${NL}${claude_tip}"
