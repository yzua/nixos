...
| -------------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `python-venv`  | `python-venv/`  | venv, ruff, black, isort, flake8, mypy, bandit, safety, vulture, pytest, coverage, jupyter, django, flask, fastapi, uvicorn, numpy, pandas, matplotlib, scipy, scikit-learn, sqlalchemy, boto3, docker, ansible, sphinx, mkdocs — see README for full list |
| `rust-stable`  | `rust-stable/`  | cargo, rustc, clippy, rustfmt, pkg-config, rustLibSrc, cargo-audit (when Cargo.lock exists)                                                                                                                                                                |
| `rust-nightly` | `rust-nightly/` | Fenix nightly toolchain, cargo, rust-src, clippy, rustfmt, pkg-config, cargo-audit (when Cargo.lock exists)                                                                                                                                                |
