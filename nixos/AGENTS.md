...
Top-level system boundary for host wiring. Shared modules now live in the repo root `nixos-modules/`; host entrypoints live in `hosts/<hostname>/configuration.nix`.
...
| Path                        | Role                                                   |
| --------------------------- | ------------------------------------------------------ |
| `../nixos-modules/`                  | Shared NixOS modules imported by every host            |
| `../nixos-modules/default.nix`       | System import hub (module aggregation order)           |
| `../nixos-modules/host-defaults.nix` | Profile defaults (`desktop`/`laptop`) for `mySystem.*` |
| `../nixos-modules/validation.nix`    | Cross-module assertions and forbidden combinations     |
