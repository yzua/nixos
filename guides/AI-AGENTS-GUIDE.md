...
Five CLI tools for AI-assisted development, all configured declaratively via `home-manager/modules/ai-agents/`:
...
| **Gastown**     | `gt`       | `~/gt/settings/config.json`               | JSON   |
...
| `ocgem`                      | `opencode_gemini`                                                  | OpenCode with Gemini Antigravity profile                             |
| `ocgpt`                      | `opencode_gpt`                                                     | OpenCode with GPT profile                                            |
| `gem`                        | `gemini --yolo`                                                    | Gemini CLI YOLO (default)                                            |
| `cx`                         | `codex --no-alt-screen --dangerously-bypass-approvals-and-sandbox` | Codex YOLO (default)                                                 |
...
| `gtra`                      | `gt rig add`                                                       | Add a rig (`gtra <name> <url>` or `gtra <name> --adopt`)            |
| `gtca`                      | `gt crew add`                                                      | Create crew workspace (`gtca <name> --rig <rig>`)                    |
...
- **Aliases** (`cl`, `clglm`, `oc`, `ocglm`, `gem`, `cx`, `occm`, etc.) are defined in `home-manager/modules/terminal/zsh/aliases.nix` as simple command mappings. All agent aliases default to YOLO/skip-permissions mode.
...
│   ├── models.nix           # Model/provider registries (OpenCode, Codex, Gemini, Gastown)
...
Multi-agent orchestration layer that coordinates multiple AI coding agents working on a codebase simultaneously. Built on top of Beads (issue tracking) and Dolt (version-controlled SQL database).

| Setting         | Value                              | Why                                     |
| --------------- | ---------------------------------- | --------------------------------------- |
| Binary          | `gt` (v0.7.0, Go, GitHub release) | Custom Nix derivation from GitHub       |
| Config          | `~/gt/settings/config.json`        | Town-level settings                     |
| Default agent   | `zai-claude`                       | Claude Code via Z.AI proxy              |
| Agent command   | `claude --dangerously-skip-permissions` | Autonomous operation for worker agents |
| API proxy       | `https://api.z.ai/api/anthropic`   | Z.AI Anthropic-compatible endpoint      |
| Haiku model     | `glm-5`                            | All model slots use GLM-5               |
| Sonnet model    | `glm-5`                            | All model slots use GLM-5               |
| Opus model      | `glm-5`                            | All model slots use GLM-5               |
| Auth            | Sops-injected at activation        | `ANTHROPIC_AUTH_TOKEN` from sops        |

#### Dependencies

| Tool      | Binary | Version | Purpose                                    |
| --------- | ------ | ------- | ------------------------------------------ |
| **Dolt**  | `dolt` | 1.59.10 | Version-controlled SQL database for Beads  |
| **Beads** | `bd`   | 0.42.0  | Git-backed issue tracking (work items)     |

All three are installed via nixpkgs unstable (`home-manager/packages/development.nix` and `home-manager/packages/custom/gastown.nix`).

#### Architecture

```text
Gastown (gt)    — Multi-agent orchestration (spawns/monitors AI agents)
    ↓
Beads (bd)      — Git-backed issue/work tracking (convoys, beads)
    ↓
Dolt            — Version-controlled SQL database backend
```
...
| -------- | ------------ | ---------------------------------------- |
| polecat  | `zai-claude` | Worker agents spawned for tasks          |
| witness  | `zai-claude` | Monitors agent lifecycle, detects stuck  |
| refinery | `zai-claude` | Merge queue processing, code integration |
| crew     | `zai-claude` | Developer workspace agents               |
...
# Add a remote project as a rig
gt rig add myproject https://github.com/you/repo.git

# Adopt a local repo as a rig (e.g. ~/System)
ln -s ~/System ~/gt/system
gt rig add system --adopt --url "$(git -C ~/System remote get-url origin)"

# Create your workspace
gt crew add yourname --rig system

# Start the Mayor (primary coordinator)
gt mayor attach     # or: gtma

# Monitor agents
gt feed             # TUI feed (or: gtf)
gt dashboard        # Web dashboard (or: gtd)
```
...
| Concept    | Description                                                   |
| ---------- | ------------------------------------------------------------- |
| **Town**   | Workspace root (`~/gt/`) containing rigs, settings, mayor     |
| **Rig**    | Project container wrapping a git repository                   |
| **Mayor**  | Primary AI coordinator (Claude Code with full context)        |
| **Polecat**| Worker agents spawned for individual tasks                    |
| **Hook**   | Git worktree-based persistent storage for agent state         |
| **Convoy** | Work tracking bundle grouping multiple beads/issues           |
| **Bead**   | Individual work item with dependencies and messaging          |

#### Config Management

Town-level config is at `~/gt/settings/config.json` (managed by Nix + sops patching). Per-rig overrides go in `~/gt/<rig>/settings/config.json` (manual).

```bash
gt config show                    # Show current config
gt config agent list              # List configured agents
gt config default-agent <name>    # Change default agent
```
...
8. `~/gt/settings/config.json` — Gastown `ANTHROPIC_AUTH_TOKEN` for zai-claude agent
...
Multi-agent at scale  -> gastown (gt) with beads for work tracking
```
