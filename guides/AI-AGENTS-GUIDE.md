...
| `ocgem` | OpenCode with Gemini Antigravity profile | Function: sets OPENCODE_CONFIG_DIR to `~/.config/opencode-gemini/` |
...
- **Functions** (`clg`, `ocg`, `ocgem`, `oc-sops`, `cl-sops`, `oc-tmux`, `qq`, `deep`, `fix`) are defined in `terminal/zsh/default.nix` — needed for env var injection, sops key loading, or multi-line logic
...
## Gemini Antigravity Profile (`ocgem`)
...
### `ocgem` — OpenCode with Gemini Antigravity
...
Gemini coding         -> ocgem (opencode with Gemini Antigravity)
